...
export class ECSComponent {
    ...
    public addToCategory(component: Component): void {
        ...
        if (!this.category.has(component.group)) {
            this.category.set(component.group, new Set());
        }
        this.category.get(component.group)!.add(component);
    }

    public removeFromCategory(component: Component): void {
        if (!component.group) return;
        const set = this.category.get(component.group);
        if (set) {
            set.delete(component);
            if (set.size === 0) {
                this.category.delete(component.group);
            }
        }
    }
