...
export function resolveOverlap(aPos: Vec2, a: Collider, bPos: Vec2, b: Collider): Vec2 | null {
  ...
  const isABox = isOfType<BoxCollider2D>(a, ComponentTypes.BoxCollider2D);
  const isBBox = isOfType<BoxCollider2D>(b, ComponentTypes.BoxCollider2D);
  const isACircle = isOfType<CircleCollider2D>(a, ComponentTypes.CircleCollider2D);
  const isBCircle = isOfType<CircleCollider2D>(b, ComponentTypes.CircleCollider2D);
