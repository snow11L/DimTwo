...
import type { SpriteRenderType } from "../../render/spriteRender/types";
import type { AnimatorState, AnimatorType } from "./types";

function startClip(animator: AnimatorType, clip: AnimationClip, lock: boolean) {
...
export function setAnimation(animator: AnimatorType, animationClip: AnimationClip, lock = false) {
  if (animator.currentClip === animationClip || animator.locked) return;
  startClip(animator, animationClip, lock);
}

export function setAnimatorState(animator: AnimatorType, newStateName: string, lock: boolean = false) {
...
export function updateSprite(animator: AnimatorType, state: AnimatorState, spriteRender: SpriteRenderType) {
...
export function getAnimatorState(animator: AnimatorType): Result<AnimatorState> {
  const controller = animator.controller;
